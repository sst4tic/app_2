...
class BagCartWidgetState extends State<BagCartWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    void changeQty({required int id, required int qty}) async {
      var url = '${Constants.API_URL_DOMAIN}action=cart_product_qty&product_id=$id&qty=$qty';
      ...
      if(body['success'] == true) {
        setState(() {
          cartList.forEach((element) {
            if(element.id == id) {
              element.qty = qty;
              cart = CartItem(items: cartList, totalSum: cart.totalSum, link: cart.link);
            }
          });
          widget.qtyChangeCallback.call();
          Func().showSnackbar(context, 'Количество товара изменено', body['success']);
        });
      } else {
        widget.qtyChangeCallback.call();
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(cartItem.name, style: const TextStyle(fontSize: 15), maxLines: 2, overflow: TextOverflow.ellipsis),
